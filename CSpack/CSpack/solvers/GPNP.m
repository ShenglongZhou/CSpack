...
if  funhd    
    ...
    if  isfield(data,'At') && ~isempty(data.At)
        funAtv = @(v)data.At*v; 
    else
        funAtv = @(v)(v'*data.A)';
    end
...
for iter = 1:maxit     
    ...
    if  normg<tol || fx < tolF  || count  || (std(OBJ)<eps*(1+obj)) 
...
function [x0,sigma,J,flag,m,alpha0,gamma,thd,disp,tol,tolF,maxit]=set_parameters(s,n,b,pars)
    ...
    sn  = s/n;
    tmp = 200*(sn>0.1)+100*(sn>0.075)+50*(sn>0.05)+20*(sn>0.025)+5*(sn<=0.025);
    thd = ceil(log2(2+s)*tmp);
